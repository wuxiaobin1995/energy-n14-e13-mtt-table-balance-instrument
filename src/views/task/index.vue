<!--
 * @Author      : Mr.bin
 * @Date        : 2023-06-21 15:10:09
 * @LastEditTime: 2023-06-21 15:41:34
 * @Description : 任务详情页
-->
<template>
  <div class="task">
          ...
          >
            <el-carousel-item v-for="(item, index) in settings" :key="index">
              <div class="box">
                ...
                <div class="item">欢迎使用台式平衡康复训练仪！</div>
              </div>
            </el-carousel-item>
          </el-carousel>
        </el-card>
    ...
    handleStart() {
      ...
      if (this.orderIdType === '评估') {
        for (let i = 0; i < settings.length; i++) {
          ...
          switch (item.pattern) {
            case '平衡能力测试':
              settingsRouter.push('test-show-left-1')
              break
            case '静态稳定测试':
              settingsRouter.push('static-stability-measure')
              break
            case '动态稳定测试':
              settingsRouter.push('dynamic-stability-measure')
              break
            default:
              break
          }
